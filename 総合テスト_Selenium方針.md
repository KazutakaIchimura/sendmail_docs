...
- **新規作成直後の一覧初回表示が古いデータのまま止まる現象**: スタッフ／事業所／利用者／送付物の一覧画面で、作成・更新直後にその場の一覧（React Queryのキャッシュ）が新しいデータを反映しないまま表示されることがあった（バックエンドのデータ自体はAPIで直接確認すると正しく存在しており、フロントエンド側の取得タイミングに起因すると推測）。対応として、行が見つからない場合に強制リロードして再取得するリトライを各Page Objectの `waitForRow`／`isRowVisible` に追加。ただし強制リロードは検索ボックスの入力内容や「すべて表示」選択などの画面側state（URLに乗らないフィルタ）も一緒に消してしまうため、それらが消えることで「絞り込みで除外されているはずの行が再表示されて誤って見つかる」という二次的な誤判定を引き起こした（`OfficeListPage`／`UserListPage`の検索フィルタテストで実際に発生）。リトライ時に直前の検索キーワード・フィルタ選択を再適用することで対応。一方 `MailSendHistoryPage` の絞り込みはコンポーネントstateで保持されており同様の対応ができないため、ここでは強制リロードによるリトライを行わない方針とした。
- **上記現象の根本原因を本番フロントエンド側で特定・全修正（2026-06-24〜25）**: `welfare-web-engineer`エージェントによるレビューで、`StaffForm.tsx`／`OfficeForm.tsx`／`UserForm.tsx`／`mail-sends/CreatePage.tsx`が、作成・編集成功時に `queryClient.invalidateQueries(...)` を**awaitせず**直後に `navigate(...)` していることが指摘された。実際に検証したところ、`await` を追加するだけでは解消しなかった。原因をさらに追ったところ、`invalidateQueries()` は既定では**現在アクティブに観測されているクエリのみ**再取得する仕組みで、`onSuccess` が呼ばれる時点では遷移先の一覧画面はまだマウントされておらずアクティブな観測者が存在しないため、`await` してもほぼ即座に（何も再取得せず）解決してしまっていた。`{ refetchType: 'all' }` を指定して未観測のキャッシュも強制的に再取得するよう4箇所を修正し、`navigate()` 前にキャッシュが最新化されるようにした。この修正により**事業所・利用者・送付物の作成フローでは現象が再現しなくなった**ことをE2E側のリトライ機構を一時的に無効化した状態で確認済み。**スタッフ作成については 2026-06-25 に別の根本原因を特定・修正**（`StaffForm.tsx` が同一 `queryKey: ['staffs']` に `enabled: false` の disabled observer を保持するため、React Query v5 の `refetchQueries` 内部の `isDisabled()` フィルタで GET がスキップされ `invalidateQueries` が即座に解決していた。`fetchQuery({ staleTime: 0 })` で observer の disabled 状態を経由せず直接フェッチするよう修正）。E2E側の強制リロードリトライ機構は安全策として引き続き維持している。
...
- 「新規作成直後の一覧初回表示が古いデータのまま止まる現象」（前述）について、事業所・利用者・送付物は本番側の `refetchType: 'all'` 修正で解消済み。**スタッフ作成も 2026-06-25 に根本原因を特定・修正済み**（`StaffForm.tsx` が同一 `queryKey: ['staffs']` に `enabled: false` の disabled observer を保持するため `invalidateQueries` の `isDisabled()` フィルタで GET がスキップされていた。`fetchQuery({ staleTime: 0 })` に置き換えて navigate 前にキャッシュを確実に最新化するよう修正）。E2E側の強制リロードリトライ機構は安全策として引き続き維持している。
