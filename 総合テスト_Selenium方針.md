...
- **上記現象の根本原因を本番フロントエンド側で特定・一部修正（2026-06-24）**: `welfare-web-engineer`エージェントによるレビューで、`StaffForm.tsx`／`OfficeForm.tsx`／`UserForm.tsx`／`mail-sends/CreatePage.tsx`が、作成・編集成功時に `queryClient.invalidateQueries(...)` を**awaitせず**直後に `navigate(...)` していることが指摘された。実際に検証したところ、`await` を追加するだけでは解消しなかった。原因をさらに追ったところ、`invalidateQueries()` は既定では**現在アクティブに観測されているクエリのみ**再取得する仕組みで、`onSuccess` が呼ばれる時点では遷移先の一覧画面はまだマウントされておらずアクティブな観測者が存在しないため、`await` してもほぼ即座に（何も再取得せず）解決してしまっていた。`{ refetchType: 'all' }` を指定して未観測のキャッシュも強制的に再取得するよう4箇所を修正し、`navigate()` 前にキャッシュが最新化されるようにした。この修正により**事業所・利用者・送付物の作成フローでは現象が再現しなくなった**ことをE2E側のリトライ機構を一時的に無効化した状態で確認済み。**ただしスタッフ作成のみ、この修正後も同じタイミング問題が再現する**（`StaffManagementE2ETest.createStaff_withValidFields_succeedsAndListed`、`LoginE2ETest.login_withDisabledAccount_showsErrorMessage`）。スタッフ作成だけがパスワードを含み登録時にBCryptハッシュ化が発生する点が他の3エンティティとの違いだが、これが原因かは未確認。本番側の `refetchType: 'all'` 修正自体は無害な改善のためそのまま適用済みだが、E2E側の強制リロードリトライ機構は安全策として維持している。
...
- 「新規作成直後の一覧初回表示が古いデータのまま止まる現象」（前述）について、事業所・利用者・送付物は本番側の `refetchType: 'all'` 修正で解消を確認済みだが、**スタッフ作成のみ原因不明のまま残存**している。E2E側の強制リロードリトライで実用上は安定して全件成功しているが、スタッフ作成時に何が他と異なるのか（BCryptハッシュ化の遅延等、未確認）の特定は今後の課題として残る。
